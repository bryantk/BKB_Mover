...
public sealed class ObjectPool : MonoBehaviour
{
	...
	Dictionary<GameObject, GameObject> classDatas = new Dictionary<GameObject, GameObject>();
	...
	/*
	//added
	public GameObject prefabByID(int i) {
		foreach (StartupPool s in startupPools) {
			if (((TypesEnum)i) == s.id)
				return s.prefab;
		}
		Debug.LogError("Prefab of id '"+i+"' aka '"+ ((TypesEnum)i).ToString() +"' does not exist on " + gameObject.name);
		return null;
	}
	*/
		...
		if (instance.pooledObjects.TryGetValue(prefab, out list))
		{
			...
			if (list.Count > 0)
			{
				...
				if (obj != null)
				{
					...
					obj.SetActive(true);
					instance.classDatas.Add(obj, prefab);
					return obj;
				}
			}
	...
	{
		GameObject prefab;
		if (instance.classDatas.TryGetValue(obj, out prefab))
			Recycle(obj, prefab);
		else 
			Object.Destroy(obj);
	}
	static void Recycle(GameObject obj, GameObject prefab)
	{
		...
		instance.classDatas.Remove(obj);
	...
	{
		foreach (var item in instance.classDatas)
	...
	{
		tempList.AddRange(instance.classDatas.Keys);
		for (int i = 0; i < tempList.Count; ++i)
			Recycle(tempList[i]);
		tempList.Clear();
	}
	...
	{
		return instance.classDatas.ContainsKey(obj);
	}
	...
	{
		int count = 0 ;
		foreach (var instancePrefab in instance.classDatas.Values)
